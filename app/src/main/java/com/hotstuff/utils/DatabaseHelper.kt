...
//    }
     ...
     * @author Cam Patten
     */
    fun addItem(item: com.hotstuff.models.Item) {
        val db = this.writableDatabase
     ...
     * @author Cam Patten
     */
    fun updateItem(item: com.hotstuff.models.Item) {
        val db = this.writableDatabase
        val cv = ContentValues()
        cv.put(COLUMN_ITEM_NAME, item.name)
        cv.put(COLUMN_ITEM_QUANTITY, item.quantity)
     ...
     * @author Cam Patten
     */
    fun addBuilding(building: com.hotstuff.models.Building) {
        val db = this.writableDatabase
        val cv = ContentValues()
     ...
     * @author Cam Patten
     */
    fun updateBuilding(building: com.hotstuff.models.Building) {
        val db = this.writableDatabase
        val cv = ContentValues()
        cv.put(COLUMN_BUILDING_NAME, building.name)
        cv.put(COLUMN_BUILDING_TYPE, building.type)
    ...
    fun getDataRange(offset: Int = 0, isSearchQuery: Boolean = false,
                     searchQuery: String? = null): ArrayList<com.hotstuff.models.Item> {
        val itemList: ArrayList<com.hotstuff.models.Item> = ArrayList()
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                val i = com.hotstuff.models.Item(
